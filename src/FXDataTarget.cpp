...
*********************************************************************************
* $Id: FXDataTarget.cpp,v 1.29 2004/02/08 17:29:06 fox Exp $                    *
********************************************************************************/
#include "xincs.h"
#include "fxver.h"
#include "fxdefs.h"
...
long FXDataTarget::onCmdValue(FXObject* sender,FXSelector sel,void*){
  ...
  for(QPtrVectorIterator<Generic::BoundFunctorV> it(upcalls); it.current(); ++it)
	  (*it.current())();
  return 1;
  }
...
Generic::BoundFunctorV *FXDataTarget::addUpcall(FXAutoPtr<Generic::BoundFunctorV> upcall)
{
	FXERRHM(PtrPtr(upcall));
	upcalls.append(PtrPtr(upcall));
	return PtrRelease(upcall);
}

/// Removes a bound functor being called when the data changes
bool FXDataTarget::removeUpcall(Generic::BoundFunctorV *upcall)
{
	return upcalls.removeRef(upcall);
}
