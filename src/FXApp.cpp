...
// Async message record
struct FXAsyncMsg {
  FXAsyncMsg    *next;              // Next async message in list
  FXObject      *source;            // Source object
  FXObject      *target;            // Receiver object
  void          *data;              // User data
  FXuint         message;           // Message sent to receiver
  };
...
// Map
FXDEFMAP(FXEventLoop) FXEventLoopMap[]={
	FXMAPFUNC(SEL_TIMEOUT,FXEventLoop::ID_ASYNCRECALC,FXEventLoop::onCmdAsyncRecalc),
};

// Implementation
FXIMPLEMENT(FXEventLoop,FXObject,FXEventLoopMap,ARRAYNUMBER(FXEventLoopMap))
static FXEventLoop *primaryEventLoop;
...
{
  if(!primaryEventLoop) primaryEventLoop=this;
  ...
  asyncmsgs=NULL;                         // Asynchronously posted messages
  asyncmsgsrecs=NULL;                     // Recycled asynchronously posted messages
  ...
  latch=CreateEvent(NULL, TRUE, FALSE, NULL); // Used to latch event loop
...
// Performs post-init setup
void FXEventLoop::setup(){

  // Root window
  root=new FXRootWindow(app, app->defaultVisual);

  }

// Allocates a per-thread static instance
void* FXEventLoop::makeStaticPtr(void* val){
  void **data=new void *;
  *data=val;
  return (void*) data;
  }
...
void FXEventLoop::deleteStaticPtr(void* ref){
  void **data=(void**) ref;
  delete data;
  }

// Retrieves the value stored in the per-thread static instance
void* FXEventLoop::getStaticPtr(const void* ref) const{
  void **data=(void**) ref;
  return *data;
  }

// Sets the value stored in the per-thread static instance
void FXEventLoop::setStaticPtr(void* ref,void* val){
  void **data=(void**) ref;
  *data=val;
  }
...
FXEventLoop::~FXEventLoop(){
  ...
  FXFREE(&e_fds);
#else
  if(latch) { CloseHandle(latch); latch=0; }
  FXFREE(&handles);
#endif
  ...
  // Free asynchronously posted messages
  while(asyncmsgs){
    a=asyncmsgs;
    asyncmsgs=asyncmsgs->next;
    delete a;
    }

  // Free recycled asynchronously posted messages
  while(asyncmsgsrecs){
    a=asyncmsgsrecs;
    asyncmsgsrecs=asyncmsgsrecs->next;
    delete a;
    }
  ...
  if(primaryEventLoop==this) primaryEventLoop=0;
  }

long FXEventLoop::onCmdAsyncRecalc(FXObject *from, FXSelector sel, void *ptr)
{
	FXWindow *w=(FXWindow *) ptr;
	//FXWindow *wshell=w->getShell();
	w->recalc();
	//wshell->layout();
	return 1;
}
...
void FXEventLoop::postAsyncMessage(FXObject* tgt,FXuint msg,void *ptr,FXObject* src){
  FXLockHold h(this);
  register FXAsyncMsg *a,**aa;
  if(asyncmsgsrecs){
    a=asyncmsgsrecs;
    asyncmsgsrecs=a->next;
    }
  else{
    a=new FXAsyncMsg;
    }
  ...
  a->message=msg;
  a->data=ptr;
  for(aa=&asyncmsgs; *aa; aa=&(*aa)->next);
  a->next=0;
  *aa=a;
  // Latch the event loop
  latchLoop();
  }


// Posts a message asynchronously
void FXEventLoop::postAsyncMessage(FXId* tgt,FXuint msg,void *ptr,FXObject* src){
  tgt->getEventLoop()->postAsyncMessage((FXObject*) tgt,msg,ptr,src);
  }


// Ask for recalc() next event cycle
void FXEventLoop::requestAsyncRecalc(FXWindow *what)
{
	FXEventLoop *ev=what->getEventLoop();
	ev->addTimeout(ev, ID_ASYNCRECALC, 1, (void*) what);
	//ev->postAsyncMessage(ev, FXSEL(SEL_COMMAND, ID_ASYNCRECALC), NULL, what);
}
...
FXApp::FXApp(const FXString& name,const FXString& vendor):registry(name,vendor){
  ...
  eventLoop=new FXEventLoop(this);        // Event Loop
...
// Find window from id
FXWindow* FXEventLoop::findWindowWithId(FXID xid) const {
  return (FXWindow*)hash.find((void*)xid);
  }


// Find window from root x,y, starting from given window
FXWindow* FXEventLoop::findWindowAt(FXint rx,FXint ry,FXID window) const {
  if(app->initialized){
#ifndef WIN32
...
//    display=GetModuleHandle(NULL);
    ...
    //wndclass.style=0;
    wndclass.lpfnWndProc=(WNDPROC) FXEventLoop::wndproc;
	...
	// Make font now FXApp is ready
	{
		...
		FXFontDesc *fonts=0;
		FXuint numfonts, f;
		if(!FXFont::listFonts(fonts, numfonts, "", FONTWEIGHT_DONTCARE, FONTSLANT_DONTCARE,
							 FONTSETWIDTH_DONTCARE, FONTENCODING_DEFAULT, FONTHINT_DONTCARE))
		{
...
/// Get event loop for callee. Create if non-existent.
FXEventLoop* FXApp::getEventLoop() const {
  if(!eventLoop) eventLoop=new FXEventLoop(const_cast<FXApp *>(this));
  return eventLoop;
}
...
void FXEventLoop::removeTimeout(FXObject* tgt,FXSelector sel){
  FXLockHold h(this);
  register FXTimer *t,**tt;
  for(tt=&timers; (t=*tt)!=NULL; tt=&t->next){
...
FXbool FXEventLoop::hasTimeout(FXObject* tgt,FXSelector sel) const {
  FXLockHold h(const_cast<FXEventLoop *>(this));
...
FXuint FXEventLoop::remainingTimeout(FXObject *tgt,FXSelector sel) {
  FXLockHold h(this);
...
void FXEventLoop::signalhandler(int sig){
  FXEventLoop *me=FXApp::instance()->getEventLoop();
  me->signals[sig].notified=TRUE;
  me->latchLoop();
  }
...
void FXEventLoop::immediatesignalhandler(int sig){
  FXEventLoop *me=FXApp::instance()->getEventLoop();
  if(me->signals[sig].target) me->signals[sig].target->handle(me->app,FXSEL(SEL_SIGNAL,me->signals[sig].message),(void*)(FXival)sig);
  }


// Add a signal message
void FXEventLoop::addSignal(FXint sig,FXObject* tgt,FXSelector sel,FXbool immediate,FXuint flags){
  void (*handler)(int);
...
void FXEventLoop::removeSignal(FXint sig){
...
void FXEventLoop::addChore(FXObject* tgt,FXSelector sel,void *ptr){
...
void FXEventLoop::removeChore(FXObject* tgt,FXSelector sel){
  register FXChore *c,**cc;
  for(cc=&chores; (c=*cc)!=NULL; cc=&c->next){
...
FXbool FXEventLoop::hasChore(FXObject* tgt,FXSelector sel) const {
...
// Add input
FXbool FXEventLoop::addInput(FXInputHandle fd,FXuint mode,FXObject *tgt,FXSelector sel){
  ...
  if(maxinput+1>=ninputs){            // Grow table of callbacks
    ...
    FXRESIZE(&handles,void*,ninputs+2);
    ninputs=ninputs+1;
    }
...
// Remove input
FXbool FXEventLoop::removeInput(FXInputHandle fd,FXuint mode){
  if(mode==INPUT_NONE) return FALSE;
#ifndef WIN32
...
// Generate SEL_LEAVE
void FXEventLoop::leaveWindow(FXWindow *window,FXWindow *ancestor){
...
// Generate SEL_ENTER
void FXEventLoop::enterWindow(FXWindow *window,FXWindow *ancestor){
  if(window && window->getParent() && window!=ancestor){
...
void FXEventLoop::removeRepaints(FXID win,FXint x,FXint y,FXint w,FXint h){
  FXRepaint *r,**rr;
  XEvent ev;
...
// be painted.
void FXEventLoop::scrollRepaints(FXID win,FXint dx,FXint dy){
  register FXRepaint *r;
...
/* 21st Feb 2004 ned: Really not sure why Jeroen hasn't done this himself :(
We check timers at the end of each Win32 dispatch event if repostimer is
non zero and we do idle processing by seeing if any work is done between
timer ticks. This solves the modal loop problem */
FXbool FXEventLoop::doIdleProcessing()
{
    // Do our chores :-)
    if(chores){
...
void FXEventLoop::doAsyncMsgs()
{
    FXLockHold h(this);
    while(asyncmsgs){
      register FXAsyncMsg* a=asyncmsgs;
      asyncmsgs=a->next;
      h.unlock();
      if(a->target && a->target->tryHandle(a->source,a->message,a->data)) refresh();
      h.relock();
      a->next=asyncmsgsrecs;
      asyncmsgsrecs=a;
      }
    // Reset latch
    resetLoopLatch();
...
FXEventLoop* FXApp::getPrimaryEventLoop()
{
...
FXbool FXApp::getNextEvent(FXRawEvent& ev,FXbool blocking){
  return getEventLoop()->getNextEvent(ev, blocking);
  }

FXbool FXApp::dispatchEvent(FXRawEvent& ev){
  return getEventLoop()->dispatchEvent(ev);
  }

// Get an event
FXbool FXEventLoop::getNextEvent(FXRawEvent& ev,FXbool blocking){
  FXbool result=TRUE;
  ...
  if(!getNextEventI(ev,FALSE)){
    ...
    if(!doIdleProcessing()) return FALSE;
    ...
    result=getNextEventI(ev,TRUE);
    }

  // Dispatch asynchronous messages (must be after message fetch)
  doAsyncMsgs();
...
void FXEventLoop::latchLoop() {
  ...
  FD_SET(latch[0], &readfds);
  if(SELECT(latch[0]+1,&readfds,&writefds,&exceptfds,&delta)) return;
...
void FXEventLoop::resetLoopLatch() {
  ...
  for(;;){
    ...
    // poll() would be easier here, but it's non-standard
    FD_SET(latch[0], &readfds);
    if(!SELECT(latch[0]+1,&readfds,&writefds,&exceptfds,&delta)) break;

    char buffer[4];
    read(latch[0], buffer, 4);
    }
  }
...
FXbool FXEventLoop::getNextEventI(FXRawEvent& ev,FXbool blocking){
  ...
  if(!initialized || !XEventsQueued((Display*)display,QueuedAfterFlush)){
    ...
    // Add latch
    FD_SET(latch[0],&readfds);
    if(latch[0]>maxfds) maxfds=latch[0];
    ...
    // Wait for any ready events or inputs
    nfds=SELECT(maxfds+1,&readfds,&writefds,&exceptfds,deltaaddr);
    ...
    if(0<=maxinput){
      ...
      for(FXInputHandle fff=0; fff<=maxinput; fff++){
        ...
        if(FD_ISSET(fff,&exceptfds)){
          if(in.excpt.target) postAsyncMessage(in.excpt.target,FXSEL(SEL_IO_EXCEPT,in.read.message),(void*)(FXival)fff,this);
          }
        }
      }
...
// Implementation of peek for event
FXbool FXEventLoop::peekEventI(){
  struct timeval delta;
  fd_set readfds;
  fd_set writefds;
  fd_set exceptfds;
...
*/
      ...
      case ButtonRelease:
        ...
        if(ev.xbutton.button==Button4 || ev.xbutton.button==Button5){     // Mouse wheel
          ...
          if(ev.xbutton.type==ButtonPress){                               // Mouse button press
            ...
            if(!event.moved && (event.time-event.click_time<app->clickSpeed) && (event.code==(FXint)event.click_button)){
              event.click_count++;
          ...
          // FIXME doesSaveUnder test should go away
            ...
            if(ev.xany.type==ButtonPress) app->beep();
            }
          event.last_x=event.win_x;
          event.last_y=event.win_y;
          }
      ...
      case SelectionRequest:
        ...
        if(ev.xselectionrequest.selection==XA_PRIMARY){
          if(selectionWindow){
            if(ev.xselectionrequest.target==app->ddeTargets){            // Request for TYPES
              FXTRACE((100,"Window %ld being requested by window %ld for SELECTION TYPES; sending %d types\n",ev.xselectionrequest.owner,ev.xselectionrequest.requestor,app->xselNumTypes));
              answer=fxsendtypes((Display*)display,ev.xselectionrequest.requestor,ev.xselectionrequest.property,app->xselTypeList,app->xselNumTypes);
              }
              ...
              FXTRACE((100,"Window %ld being requested by window %ld for SELECTION DATA of type %ld; sending %d bytes\n",ev.xselectionrequest.owner,ev.xselectionrequest.requestor,ev.xselectionrequest.target,app->ddeSize));
              answer=fxsenddata((Display*)display,ev.xselectionrequest.requestor,ev.xselectionrequest.property,ev.xselectionrequest.target,app->ddeData,app->ddeSize);
              FXFREE(&app->ddeData);
              app->ddeData=NULL;
              app->ddeSize=0;
              }
        ...
        else if(ev.xselectionrequest.selection==app->xcbSelection){
          if(clipboardWindow){
            if(ev.xselectionrequest.target==app->ddeTargets){            // Request for TYPES
              FXTRACE((100,"Window %ld being requested by window %ld for CLIPBOARD TYPES; sending %d types\n",ev.xselectionrequest.owner,ev.xselectionrequest.requestor,app->xcbNumTypes));
              answer=fxsendtypes((Display*)display,ev.xselectionrequest.requestor,ev.xselectionrequest.property,app->xcbTypeList,app->xcbNumTypes);
              }
            else{                                                   // Request for DATA
        ...
        else if(ev.xselectionrequest.selection==app->xdndSelection){
          if(dragWindow){
            if(ev.xselectionrequest.target==app->ddeTargets){            // Request for TYPES
              FXTRACE((100,"Window %ld being requested by window %ld for XDND TYPES; sending %d types\n",ev.xselectionrequest.owner,ev.xselectionrequest.requestor,app->xdndNumTypes));
              answer=fxsendtypes((Display*)display,ev.xselectionrequest.requestor,ev.xselectionrequest.property,app->xdndTypeList,app->xdndNumTypes);
              }
            else{                                                   // Request for DATA
      ...
      case ClientMessage:
        ...
        // WM_PROTOCOLS
        if(ev.xclient.message_type==app->wmProtocols){
          if((FXID)ev.xclient.data.l[0]==app->wmDeleteWindow){           // WM_DELETE_WINDOW
          ...
          else if((FXID)ev.xclient.data.l[0]==app->wmQuitApp){           // WM_QUIT_APP
            event.type=SEL_CLOSE;
            if(!invocation || invocation->modality==MODAL_FOR_NONE || (invocation->window && invocation->window->isOwnerOf(window))){
              if(window->handle(this,FXSEL(SEL_CLOSE,0),&event)) refresh();
              }
          ...
          else if((FXID)ev.xclient.data.l[0]==app->wmTakeFocus){         // WM_TAKE_FOCUS
        ...
        // XDND Enter from source
        else if(ev.xclient.message_type==app->xdndEnter){
          ...
          if(ev.xclient.data.l[1]&1){
            ...
            app->ddeNumTypes=0;
            if(ev.xclient.data.l[2]){app->ddeTypeList[0]=ev.xclient.data.l[2];app->ddeNumTypes++;}
            if(ev.xclient.data.l[3]){app->ddeTypeList[1]=ev.xclient.data.l[3];app->ddeNumTypes++;}
            if(ev.xclient.data.l[4]){app->ddeTypeList[2]=ev.xclient.data.l[4];app->ddeNumTypes++;}
            }
          }

        // XDND Leave from source
        else if(ev.xclient.message_type==app->xdndLeave){
        ...
        // XDND Position from source
        else if(ev.xclient.message_type==app->xdndPosition){
          ...
          if((FXID)ev.xclient.data.l[4]==app->xdndActionCopy) app->ddeAction=DRAG_COPY;
          else if((FXID)ev.xclient.data.l[4]==app->xdndActionMove) app->ddeAction=DRAG_MOVE;
          else if((FXID)ev.xclient.data.l[4]==app->xdndActionLink) app->ddeAction=DRAG_LINK;
          else if((FXID)ev.xclient.data.l[4]==app->xdndActionPrivate) app->ddeAction=DRAG_PRIVATE;
          else app->ddeAction=DRAG_COPY;
          app->ansAction=DRAG_REJECT;
          app->xdndWantUpdates=TRUE;
          app->xdndRect.x=event.root_x;
          app->xdndRect.y=event.root_y;
          app->xdndRect.w=1;
          app->xdndRect.h=1;
          ...
          se.xclient.message_type=app->xdndStatus;
          ...
          if(app->ansAction!=DRAG_REJECT) se.xclient.data.l[1]|=1;  // Target accepted
          if(app->xdndWantUpdates) se.xclient.data.l[1]|=2;         // Target wants continuous position updates
          se.xclient.data.l[2]=MKUINT(app->xdndRect.y,app->xdndRect.x);
          se.xclient.data.l[3]=MKUINT(app->xdndRect.h,app->xdndRect.w);
          if(app->ansAction==DRAG_COPY) se.xclient.data.l[4]=app->xdndActionCopy; // Drag and Drop Action accepted
          else if(app->ansAction==DRAG_MOVE) se.xclient.data.l[4]=app->xdndActionMove;
          else if(app->ansAction==DRAG_LINK) se.xclient.data.l[4]=app->xdndActionLink;
          else if(app->ansAction==DRAG_PRIVATE) se.xclient.data.l[4]=app->xdndActionPrivate;
        ...
        // XDND Drop from source
          ...
          if(dropWindow){
            ...
            if(dropWindow->handle(this,FXSEL(SEL_DND_DROP,0),&event)){
              ...
              if(app->ansAction==DRAG_COPY) se.xclient.data.l[2]=app->xdndActionCopy;             // Action performed by target
              else if(app->ansAction==DRAG_MOVE) se.xclient.data.l[2]=app->xdndActionMove;        // For now, same as our last
              else if(app->ansAction==DRAG_LINK) se.xclient.data.l[2]=app->xdndActionLink;        // answer from the xdndPosition
        ...
        // XDND Status from target
        else if(ev.xclient.message_type==app->xdndStatus){
          ...
          app->ansAction=DRAG_REJECT;
          if(ev.xclient.data.l[1]&1){
            if((FXID)ev.xclient.data.l[4]==app->xdndActionCopy) app->ansAction=DRAG_COPY;
            else if((FXID)ev.xclient.data.l[4]==app->xdndActionMove) app->ansAction=DRAG_MOVE;
            else if((FXID)ev.xclient.data.l[4]==app->xdndActionLink) app->ansAction=DRAG_LINK;
            else if((FXID)ev.xclient.data.l[4]==app->xdndActionPrivate) app->ansAction=DRAG_PRIVATE;
            }
          app->xdndWantUpdates=ev.xclient.data.l[1]&2;
          app->xdndRect.x=((FXuint)ev.xclient.data.l[2])>>16;
          app->xdndRect.y=((FXuint)ev.xclient.data.l[2])&0xffff;
          app->xdndRect.w=((FXuint)ev.xclient.data.l[3])>>16;
          app->xdndRect.h=((FXuint)ev.xclient.data.l[3])&0xffff;
          app->xdndStatusReceived=TRUE;
          app->xdndStatusPending=FALSE;
          FXTRACE((100,"DNDStatus from remote window %ld action=%d rect=%d,%d,%d,%d updates=%d\n",ev.xclient.data.l[0],app->ansAction,app->xdndRect.x,app->xdndRect.y,app->xdndRect.w,app->xdndRect.h,app->xdndWantUpdates));
          }
        return TRUE;
...
long FXApp::dispatchEvent(FXID hwnd,unsigned int iMsg,unsigned int wParam,long lParam){
  return getEventLoop()->dispatchEvent(hwnd,iMsg,wParam,lParam);
  }
...
// Implementation of fetching an event. Returns false if no event
FXbool FXEventLoop::getNextEventI(FXRawEvent& msg,FXbool blocking){
  ...
  if(WAIT_OBJECT_0<=signalled && signalled<WAIT_OBJECT_0+allinputs){
    ...
    for(FXuint i=0; i<=(FXuint) maxinput; i++){
      ...
      if((i==signalled-WAIT_OBJECT_0) || (WaitForSingleObject(fff,0)==WAIT_OBJECT_0)){
        ...
        if(in.read.target)  postAsyncMessage(in.read.target, FXSEL(SEL_IO_READ,in.read.message),   (void*)(FXival)fff,this);
        if(in.write.target) postAsyncMessage(in.write.target,FXSEL(SEL_IO_WRITE,in.write.message), (void*)(FXival)fff,this);
        if(in.excpt.target) postAsyncMessage(in.excpt.target,FXSEL(SEL_IO_EXCEPT,in.excpt.message),(void*)(FXival)fff,this);
        }
      }
    }
...
// Dispatch event to widget
FXbool FXEventLoop::dispatchEvent(FXRawEvent& msg){
  DispatchMessage(&msg);
...
void FXEventLoop::flush(FXbool sync){
  if(initialized){
#ifndef WIN32
    if(sync)
      XSync((Display*)display,FALSE);
...
FXint FXEventLoop::runWhileEvents(){
  FXInvocation inv(&invocation,MODAL_FOR_NONE,NULL);
...
// Run event loop while events are available
FXint FXEventLoop::runModalWhileEvents(FXWindow* window){
...
FXbool FXEventLoop::runOneEvent(FXbool blocking){
  FXRawEvent ev;
...
FXbool FXEventLoop::isModal(FXWindow *window) const {
...
FXModality FXEventLoop::getModality() const {
...
void FXEventLoop::stopModal(FXWindow* window,FXint value){
  register FXInvocation* inv;
  if(isModal(window)){
    for(inv=invocation; inv; inv=inv->upper){
      inv->done=TRUE;
...
long FXApp::onCmdHover(FXObject*,FXSelector,void*){
  ...
  FXWindow *window;
  if(!me->mouseGrabWindow && me->cursorWindow && me->cursorWindow!=me->root){
    me->root->getCursorPosition(x,y,buttons);
    if((window=findWindowAt(x,y))==NULL || !window->getShell()->containsChild(me->cursorWindow)){
      me->event.type=SEL_LEAVE;
      me->event.root_x=x;
      me->event.root_y=y;
      me->leaveWindow(me->cursorWindow,me->root);
      return 0;
      }
    }
  me->addTimeout(this,ID_HOVER,200);
  return 0;
  }
...
long CALLBACK FXEventLoop::wndproc(FXID hwnd,unsigned int iMsg,unsigned int wParam,long lParam){
  FXEventLoop *me=FXApp::instance()->getEventLoop();
  long ret=me->dispatchEvent(hwnd,iMsg,wParam,lParam);
  if(me->repostimer)
  {
	  me->msgloopcount++;
	  me->doTimers();
...
long FXEventLoop::dispatchEvent(FXID hwnd,unsigned int iMsg,unsigned int wParam,long lParam){
  ...
  switch(iMsg){
      ...
      if(keyboardGrabWindow){
        ...
        if(keyWindow){
          ...
          if(!invocation || invocation->modality==MODAL_FOR_NONE || (invocation->window && invocation->window->isOwnerOf(keyWindow)) || keyWindow->getShell()->doesSaveUnder()){
            ...
            if(event.type==SEL_KEYPRESS) app->beep();
            }
          }
        }
      return 0;
    ...
    case WM_MOUSEMOVE:
      ...
      // Reset hover timer
	  addTimeout(this,FXApp::ID_HOVER,200);
...
// FIXME Does window still exist?
    ...
    case WM_SETCURSOR:
      if(app->waitCount){
        SetCursor((HCURSOR)app->waitCursor->id());                     // Show wait cursor
        return 0;
        }
    ...
    case WM_SYSCOMMAND:         // This pops down the menupane when clicking in non-client area
      ...
      return DefWindowProc((HWND)hwnd,iMsg,wParam,lParam);
    // Added 21st Feb 2004 ned: Have chores done by timer during repositions
    case WM_ENTERSIZEMOVE:
		{
			if(!repostimer)
			{
				msgloopcount=tickmsgloopcount=0;
    ...
    case WM_TIMER:
		{
			if(repostimer)
			{
				if(msgloopcount-tickmsgloopcount==1)
				{	// Nothing was done between the two timer msgs
					doIdleProcessing();
				}
				tickmsgloopcount=msgloopcount;
				return 0;
			}
			return DefWindowProc((HWND)hwnd,iMsg,wParam,lParam);
		}
    ...
    case WM_DND_ENTER:
      ...
      if(hMap){
        ...
        if(dragtypes){
          if(FXMALLOC(&app->ddeTypeList,FXDragType,dragtypes[0])){
            memcpy(app->ddeTypeList,&dragtypes[1],dragtypes[0]*sizeof(FXDragType));
            app->ddeNumTypes=dragtypes[0];
            }
...
dengo:if(app->ddeTypeList){FXFREE(&app->ddeTypeList);app->ddeNumTypes=0;}
    ...
    case WM_DND_POSITION_PRIVATE:
      ...
      app->ddeAction=(FXDragAction)(iMsg-WM_DND_POSITION_REJECT);    // Action encoded in message
      app->ansAction=DRAG_REJECT;
      app->xdndRect.x=event.root_x;
      app->xdndRect.y=event.root_y;
      app->xdndRect.w=1;
      ...
      PostMessage((HWND)app->xdndSource,WM_DND_STATUS_REJECT+app->ansAction,MAKELONG(app->xdndRect.x,app->xdndRect.y),MAKELONG(app->xdndRect.w,app->xdndRect.h));
      return 0;
    ...
    case WM_DND_STATUS_PRIVATE:
      app->ansAction=(FXDragAction)(iMsg-WM_DND_STATUS_REJECT);
      app->xdndRect.x=(int)((short)LOWORD(wParam));
      app->xdndRect.y=(int)((short)HIWORD(wParam));
      app->xdndRect.w=(int)((short)LOWORD(lParam));
      app->xdndRect.h=(int)((short)HIWORD(lParam));
      app->xdndStatusReceived=TRUE;
      app->xdndStatusPending=FALSE;
      FXTRACE((100,"DNDStatus from remote window action=%d rect x=%d y=%d w=%d h=%d\n",app->ansAction,app->xdndRect.x,app->xdndRect.y,app->xdndRect.w,app->xdndRect.h));
      return 0;
...
// Error beep
void FXApp::errorBeep(){
  if(initialized){
    FXTRACE((100,"Error beep\n"));
#ifndef WIN32
    XBell((Display*)display,0);
#else
...
// Set root Window
void FXEventLoop::setRootWindow(FXRootWindow* rt){
...
#else
      if(getCursorWindow()){
        SetCursor((HCURSOR)getCursorWindow()->getDefaultCursor()->id());
        }
#endif
      }
    }
