...
void FXEventLoop::selectionSetData(const FXWindow*,FXDragType,FXuchar* data,FXuint size){
  FXFREE(&app->ddeData);
  app->ddeData=data;
  app->ddeSize=size;
  }
...
void FXEventLoop::selectionGetData(const FXWindow* window,FXDragType type,FXuchar*& data,FXuint& size){
  ...
  if(selectionWindow){
    ...
    answer=fxsendrequest((Display*)display,window->id(),XA_PRIMARY,app->ddeAtom,type,event.time);
    fxrecvdata((Display*)display,window->id(),answer,app->ddeIncr,type,data,size);
    }
  }
...
void FXEventLoop::selectionGetTypes(const FXWindow* window,FXDragType*& types,FXuint& numtypes){
  ...
  numtypes=0;
  if(selectionWindow){
    FXMEMDUP(&types,app->xselTypeList,FXDragType,app->xselNumTypes);
...
void FXEventLoop::clipboardSetData(const FXWindow*,FXDragType,FXuchar* data,FXuint size){
  FXFREE(&app->ddeData);
  app->ddeData=data;
  app->ddeSize=size;
  }
...
void FXEventLoop::clipboardGetData(const FXWindow* window,FXDragType type,FXuchar*& data,FXuint& size){
  FXID answer;
  data=NULL;
  size=0;
  if(clipboardWindow){
    ...
    answer=fxsendrequest((Display*)display,window->id(),app->xcbSelection,app->ddeAtom,type,event.time);
    fxrecvdata((Display*)display,window->id(),answer,app->ddeIncr,type,data,size);
    }
  }
...
void FXEventLoop::clipboardGetTypes(const FXWindow* window,FXDragType*& types,FXuint& numtypes){
  FXID answer;
  types=NULL;
  numtypes=0;
  if(clipboardWindow){
...
void FXEventLoop::dragdropSetData(const FXWindow*,FXDragType,FXuchar* data,FXuint size){
  FXFREE(&app->ddeData);
  app->ddeData=data;
  app->ddeSize=size;
  }
...
void FXEventLoop::dragdropGetData(const FXWindow* window,FXDragType type,FXuchar*& data,FXuint& size){
  FXID answer;
  data=NULL;
  size=0;
  if(dragWindow){
    ...
    answer=fxsendrequest((Display*)display,window->id(),app->xdndSelection,app->ddeAtom,type,event.time);
    fxrecvdata((Display*)display,window->id(),answer,app->ddeIncr,type,data,size);
    }
  }
...
// Change CLIPBOARD selection data
void FXEventLoop::clipboardSetData(const FXWindow*,FXDragType type,FXuchar* data,FXuint size){
...
void FXEventLoop::dragdropGetData(const FXWindow* window,FXDragType type,FXuchar*& data,FXuint& size){
  ...
  if(dragWindow){
    ...
    answer=fxsendrequest((HWND)app->xdndSource,(HWND)window->id(),(WPARAM)type);
