...
*********************************************************************************
* $Id: FXRootWindow.cpp,v 1.31 2004/09/17 07:46:21 fox Exp $                    *
********************************************************************************/
#include "xincs.h"
#include "fxver.h"
#include "fxdefs.h"
...
FXRootWindow::~FXRootWindow(){
  // Delete normal children only then popups
  register FXWindow *w, *child;
  for(child=getFirst(); (w=child); ) {
    child=child->getNext();
	if(!w->isPopup()){
      delete w;
      // Must restart as children may have been deleted
      child=getFirst();
      }
  }
