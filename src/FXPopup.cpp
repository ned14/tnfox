...
void FXPopup::show(){
  if(!(flags&FLAG_SHOWN)){
    FXShell::show();
    prevActive=getEventLoop()->popupWindow;
    if(prevActive) prevActive->nextActive=this;
    getEventLoop()->popupWindow=this;
    setFocus();
    }
  }
...
void FXPopup::hide(){
  if(flags&FLAG_SHOWN){
    FXShell::hide();
    if(getEventLoop()->popupWindow==this) getEventLoop()->popupWindow=prevActive;
