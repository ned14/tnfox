...
* Tiger hash algorithm (C) Ross Anderson and Eli Biham                          *
*********************************************************************************
...
#define RANDOMNESS_SIZE 1025		// 8192+8 bits
#define RANDOMNESS_TICK 1000		// How often the background thread ticks
...
#endif
void *operator new(size_t size, const FX::Secure::heap &) throw(std::bad_alloc)
{
	void *ret;
	if(!(ret=FX::Secure::mallocI(size))) throw std::bad_alloc();
	return ret;
}
void *operator new[](size_t size, const FX::Secure::heap &) throw(std::bad_alloc)
{
...
void operator delete(void *p, const FX::Secure::heap &) throw()
{
	FX::Secure::free(p);
}
void operator delete[](void *p, const FX::Secure::heap &) throw()
...
static struct SecureHeapInit : public QMutex
{
	...
	FXuint pageSizeM1;
	static void wipeHeap(bool fatal)
	{
		FXDELETE(secureheap); secureheap=(FXMemoryPool *) -1;
		if(fatal) fxmessage("Deleted secure heap\n");
		mlock=FXProcess_MemLock();
	}
	...
	inline void lock(void *ptr, FXuval size)
	{
		...
		{
			void *a=FXMIN(ptr, mlock.location());
#ifdef DEBUG
			FXuval l=(FXuval) FXMAX(cendptr, endptr) - (FXuval) a;
#else
			// Round to page sizes
			a=(void *)(((FXuval)a) & ~pageSizeM1);
			FXuval l=(FXuval) FXMAX(cendptr, endptr) - (FXuval) a;
			l=(l+pageSizeM1) & ~pageSizeM1;
#endif
			mlock=FXProcess_MemLock(a, l);
		}
	}
} secureheapinit;
FXProcess_MemLock SecureHeapInit::mlock;

#ifdef DEBUG
#define SECUREHEADERC 2
#else
#define SECUREHEADERC 1
#endif
#define SECUREHEADERSIZE (SECUREHEADERC*sizeof(FXuval))
#define SECUREHEADERMAGIC (*((FXuval *)"TFSCTFSC"))
void *mallocI(size_t size) throw()
{
	if(!secureheap) secureheap=::new FXMemoryPool(SECUREHEAPMAXLEN);
	void *ret=secureheap->malloc(size+SECUREHEADERSIZE);
	if(!ret) return 0;
...
void *callocI(size_t no, size_t size) throw()
{
	if(!secureheap) secureheap=::new FXMemoryPool(SECUREHEAPMAXLEN);
	void *ret=secureheap->malloc((no*size)+SECUREHEADERSIZE);
	if(!ret) return 0;
	FXuval *_ret=(FXuval *) ret;
	_ret[0]=size;
#ifdef DEBUG
...
class RandomnessPrivate : public QThread, public QMutex
{
	...
	struct SharedMemLayout
	{
		...
		FXuval offset;			// Where in data last write
		FXuint size;			// Data written till max of RANDOMNESS_SIZE
		FXuchar data[RANDOMNESS_SIZE];
		FXuint padding;
	} *data;
	FXuval offset;
	PDH_HQUERY query;
	struct CounterInfo
	{
		...
		bool hadError;
		PDH_HCOUNTER handle;
		PDH_RAW_COUNTER data;
		CounterInfo(PDH_HCOUNTER h, const FXString &_path) : path(_path), hadError(false), handle(h), data() { }
	};
	QValueList<CounterInfo> counters;
	FXulong randomness;
	int randomnesslen;
	POINT cursorpos;

#define FXERRHPDH(exp) { PDH_STATUS __status=(exp); if(IsErrorSeverity(__status)) FXERRHWIN(ERROR_SUCCESS==__status); }
	void addCounter(const FXString &object, const FXString &counter)
	{
...
#endif
		{
			...
			if(0==ret)
			{
				FXERRHPDH(PdhAddCounter(query, str, 0, &ch));
				counters.append(CounterInfo(ch, str));
			}
		}
	}
	void addRandomness(FXulong diff) throw()
	{
		if(!data) return;
		int n=0;
		while(!(diff & 1)) diff>>=1;
		for(n=63; !(diff>>n); n--); n++;
		randomness|=(diff<<randomnesslen);
		randomnesslen+=n;
		while(randomnesslen>=8)
		{
			randomnesslen-=8;
			data->data[data->offset++]=(FXuchar)((randomness>>randomnesslen) & 0xff);
			randomness>>=8;
...
public:
	...
	void readBlock(FXuchar *buffer, FXuval length)
	{
		...
		while(length)
		{
			...
			FXuval left=data->size-offset;
			FXuval tocopy=FXMIN(left, length);
			memcpy(buffer, data->data+offset, tocopy);
			buffer+=tocopy; length-=tocopy; offset+=tocopy;
		}
#endif
	}
	FXuval size()
	...
	void run()
	{
		...
		for(;;)
		{
			...
			if(amMaster || now-data->lastupdate>RANDOMNESS_TICK)
			{
				data->lock.lock();
				if(amMaster || now-data->lastupdate>RANDOMNESS_TICK)	// In case master hasn't actually died
				{
					amMaster=true;
					data->usercountlatch=data->usercount.swap(0);
					data->lastupdate=now;
				}
				data->lock.unlock();
				// Here we go ...
				if(!query)
				{
					FXERRHPDH(PdhOpenQuery(NULL, 0, &query));
					addCounter("PhysicalDisk(_Total)", "Disk Bytes/sec");
					addCounter("PhysicalDisk(_Total)", "Avg. Disk Queue Length");
					addCounter("PhysicalDisk(_Total)", "Avg. Disk sec/Transfer");
					addCounter("PhysicalDisk(_Total)", "% Disk Time");

					addCounter("Network Interface(*)", "Bytes Total/sec");

					addCounter("Processor(_Total)", "Interrupts/sec");
					addCounter("System", "System Calls/sec");
				}
				FXERRHPDH(PdhCollectQueryData(query));
				for(QValueList<CounterInfo>::iterator it=counters.begin(); it!=counters.end(); ++it)
				{
					CounterInfo &ci=*it;
					PDH_RAW_COUNTER newdata;
					FXERRHPDH(PdhGetRawCounterValue(ci.handle, NULL, &newdata));
					if(!newdata.CStatus)
					{
						FXulong diff=newdata.FirstValue ^ ci.data.FirstValue;
					...
					{
						const char *cause=0;
						switch(newdata.CStatus)
						{
						case PDH_CSTATUS_NO_INSTANCE:
							{
								cause="The specified instance is not present";
								break;
							}
						}
						fxmessage("WARNING: Counter %s read failed, cause is 0x%x (%s)\n", ci.path.text(), newdata.CStatus, cause);
						ci.hadError=true;
					}
				}
				//addRandomness(GetTickCount());
				for(int parttick=0; parttick<10; parttick++)
				{
					FXulong diff=0;
					POINT p;
	...
	void *cleanup()
	{
		...
		if(query)
		{
			FXERRHWIN(ERROR_SUCCESS==PdhCloseQuery(query));
			query=0;
		}
#endif
		return 0;
...
static FXProcess_StaticInit<RandomnessPrivate> staticrandominit("Randomness");

FXuval Randomness::readBlock(FXuchar *buffer, FXuval length)
{
	myrandomness->readBlock(buffer, length);
	return length;
}

FXuval Randomness::size()
{
	return myrandomness->size();
}

//*******************************************************************************
/* Below follows more or less the reference implementation of the Tiger hash
algorithm. Changes made include making it use FOX typedef's rather than its own
and C++-ising it. Otherwise it is unchanged. See http://www.cs.technion.ac.il/~biham/
and http://www.cl.cam.ac.uk/users/rja14/ for more information.
*/
static const FXulong table[4*256] = {
...
#define round(a,b,c,x,mul) \
      c ^= x; \
      a -= t1[(FXuchar)(c)] ^ \
           t2[(FXuchar)(((FXuint)(c))>>(2*8))] ^ \
	   t3[(FXuchar)((c)>>(4*8))] ^ \
           t4[(FXuchar)(((FXuint)((c)>>(4*8)))>>(2*8))] ; \
      b += t4[(FXuchar)(((FXuint)(c))>>(1*8))] ^ \
           t3[(FXuchar)(((FXuint)(c))>>(3*8))] ^ \
	   t2[(FXuchar)(((FXuint)((c)>>(4*8)))>>(1*8))] ^ \
           t1[(FXuchar)(((FXuint)((c)>>(4*8)))>>(3*8))]; \
      b *= mul;
#endif
...
#define compress \
      save_abc \
      for(pass_no=0; pass_no<mypasses; pass_no++) { \
...
{ \
  register FXulong a, b, c, tmpa; \
  FXulong aa, bb, cc; \
  register FXulong x0, x1, x2, x3, x4, x5, x6, x7; \
  register FXuint i; \
  FXuint pass_no; \
\
  a = state[0]; \
