...
public:
  ...
  */
  ...
  static void int_throwOSError(const char *file, int lineno, int code, FXuint flags);
...
Use this macro to generically indicate a parameter exceedin te permitted range. As this is a common
...
*/
#ifdef DEBUG
#define FXERRHPTR(exp, flags)	if(!(exp) || FXException::int_testCondition()) FXERRGPTR(flags)
#else
...
*/
#ifdef DEBUG
#define FXERRHM(exp)		do { try { if(!FXException::int_testCondition() && (exp)) break; } catch(const std::bad_alloc &) { } FXERRGM; } while(0)
#else
...
*/
#ifdef DEBUG
#define FXERRHIO(exp)		if(-1==(int)(exp) || FXException::int_testCondition()) FXERRGIO(strerror(errno))
#else
