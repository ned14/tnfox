...
*********************************************************************************
* $Id: TextWindow.cpp,v 1.125.2.1 2006/06/08 13:09:06 fox Exp $                     *
********************************************************************************/
#include "fx.h"
#include "fxkeys.h"
#include <new>
...
FXbool TextWindow::loadFile(const FXString& file){
  ...
  // Opened file?
  if(!textfile.isOpen()){
    FXMessageBox::error(this,MBOX_OK,tr("Error Loading File").text(),tr("Unable to open file: %s").text(),file.text());
    return FALSE;
    }
  ...
  if(!FXMALLOC(&text,FXchar,size)){
    FXMessageBox::error(this,MBOX_OK,tr("Error Loading File").text(),tr("File is too big: %s (%d bytes)").text(),file.text(),size);
    return FALSE;
    }
  ...
  if(n<0){
    FXFREE(&text);
    FXMessageBox::error(this,MBOX_OK,tr("Error Loading File").text(),tr("Unable to read file: %s").text(),file.text());
    return FALSE;
    }
...
FXbool TextWindow::insertFile(const FXString& file){
  ...
  if(!fp){
    FXMessageBox::error(this,MBOX_OK,tr("Error Inserting File").text(),tr("Unable to open file: %s").text(),file.text());
    return FALSE;
    }
  ...
  // Make buffer to load file
  if(!FXMALLOC(&text,FXchar,size)){
    FXMessageBox::error(this,MBOX_OK,tr("Error Inserting File").text(),tr("File is too big: %s (%d bytes)").text(),file.text(),size);
    fclose(fp);
    return FALSE;
    }
...
FXbool TextWindow::saveFile(const FXString& file){
  ...
  // Opened file?
  if(!textfile.isOpen()){
    FXMessageBox::error(this,MBOX_OK,tr("Error Saving File").text(),tr("Unable to open file: %s").text(),file.text());
    return FALSE;
    }
  ...
  if(!FXMALLOC(&text,FXchar,size+1)){
    FXMessageBox::error(this,MBOX_OK,tr("Error Saving File").text(),tr("File is too big: %s").text(),file.text());
    return FALSE;
    }
  ...
  if(n!=size){
    FXMessageBox::error(this,MBOX_OK,tr("Error Saving File").text(),tr("File: %s truncated.").text(),file.text());
    return FALSE;
    }
...
FXbool TextWindow::extractFile(const FXString& file){
  ...
  // Opened file?
  if(!textfile.isOpen()){
    FXMessageBox::error(this,MBOX_OK,tr("Error Extracting File").text(),tr("Unable to open file: %s").text(),file.text());
    return FALSE;
    }
  ...
  // Alloc buffer
  if(!FXMALLOC(&text,FXchar,size+1)){
    FXMessageBox::error(this,MBOX_OK,tr("Error Extracting File").text(),tr("File is too big: %s").text(),file.text());
    return FALSE;
    }
  ...
  // Were we able to write it all?
  if(n!=size){
    FXMessageBox::error(this,MBOX_OK,tr("Error Extracting File").text(),tr("File: %s truncated.").text(),file.text());
    return FALSE;
    }
...
long TextWindow::onCmdReopen(FXObject*,FXSelector,void*){
  if(isModified()){
    if(FXMessageBox::question(this,MBOX_YES_NO,tr("Document was changed").text(),tr("Discard changes to this document?").text())==MBOX_CLICKED_NO) return 1;
    }
  loadFile(filename);
  return 1;
  }
...
long TextWindow::onCmdExtractFile(FXObject*,FXSelector,void*){
  ...
  if(savedialog.execute()){
    ...
    if(FXStat::exists(file)){
      if(MBOX_CLICKED_NO==FXMessageBox::question(this,MBOX_YES_NO,tr("Overwrite Document").text(),tr("Overwrite existing document: %s?").text(),file.text())) return 1;
      }
    extractFile(file);
    }
  return 1;
...
FXbool TextWindow::saveChanges(){
  ...
  writeView(filename);
  if(isModified()){
    answer=FXMessageBox::question(this,MBOX_YES_NO_CANCEL,tr("Unsaved Document").text(),tr("Save %s to file?").text(),FXPath::name(filename).text());
    ...
    if(answer==MBOX_CLICKED_YES){
      ...
      if(!filenameset){
        ...
        file=savedialog.getFilename();
        if(FXStat::exists(file)){
          if(MBOX_CLICKED_NO==FXMessageBox::question(this,MBOX_YES_NO,tr("Overwrite Document").text(),tr("Overwrite existing document: %s?").text(),file.text())) return FALSE;
          }
        }
      saveFile(file);
      }
...
long TextWindow::onFocusIn(FXObject* sender,FXSelector sel,void* ptr){
  ...
  if(warnchanged && filetime.value!=0){
    t=FXStat::modified(filename);
    if(t.value && t!=filetime){
      filetime=t;
      if(MBOX_CLICKED_OK==FXMessageBox::warning(this,MBOX_OK_CANCEL,tr("File Was Changed").text(),tr("%s\nwas changed by another program. Reload this file from disk?").text(),filename.text())){
